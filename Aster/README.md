...
- **Rebuild Nexus from Chat** — replace a chat's Memory Nexus with a clean version distilled by your own LLM (copy the built-in prompt, run it on your chat export, paste the result back). It backs up the old Nexus first, with one-click restore.
- **Thinking Template Override** — use your own thinking template instead of the bot's, set per chat (each chat keeps its own, loaded automatically when you switch). Save favourites as presets.
- **Hide from AI** — hide one message from the AI's context in a chat (e.g. a bugged duplicate you can't delete) without removing it. It collapses to a small bar and is stripped from what the model sees, until it scrolls out of context.
