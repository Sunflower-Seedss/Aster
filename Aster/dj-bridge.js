...
(function () {
  ...
  function respond(reqId, result) {
    window.postMessage({ source: 'djt-bridge', reqId: reqId, result: result }, location.origin);
  }

  window.addEventListener('message', function (ev) {
    if (ev.source !== window) return;
    if (ev.origin !== location.origin) return;   // only accept same-origin (the DJ page) requests
    var d = ev.data;
  ...
  window.postMessage({ source: 'djt-bridge', ready: true }, location.origin);
})();
