...
- **Analysis logic** (`lbAnalyze`): collapses blank lines (`\n{2,}`→`\n`) → direct trigger matches (word-boundary, case-insensitive) → BFS cascade through activated entries' bodies, skipping wrapped triggers → sort pinned-first then weight desc → estimate tokens (`length/4`) and cut anything past the **1500** token budget.
- **Wrapped-trigger handling (DJ-accurate):** `lbFindMatches` treats `_` as a **word character** (boundary class `[a-z0-9_]`), so a trigger touching `_` on EITHER side has no word boundary and won't match — this makes the underscore-wrap convention work, **including multi-word wraps** like `_Imperium Divinum_` where an inner trigger ("Imperium") is followed by a space but preceded by `_`. (Fixed 2026-06-04: the old `[a-z0-9]` boundary + `lbFindUnwrapped`'s adjacent-pair check only caught single-word wraps `_word_`, so multi-word wraps leaked spurious cascades — verified against a real user lorebook.) `lbFindUnwrapped` still strips the explicit pair-wraps `/x/` `-x-` `<x>` (used by both direct and cascade via `lbDirectHits`/`lbCascadeHits`). NOTE: the Lorebook Workshop only ever wraps with underscores, so it's fully consistent.
