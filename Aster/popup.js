...
var VIS_SECTIONS = [
  ...
  { key:'nav',          label:'Navigation',             group:'Chat Tools' },
  { key:'scratch',      label:'User Input Recovery',    group:'Chat Tools' },
  { key:'nexustool',    label:'Rebuild Nexus from Chat', group:'Chat Tools' },
  { key:'thinking',     label:'Thinking Template Override', group:'Chat Tools' },
  { key:'hidemsg',      label:'Hide from AI',           group:'Chat Tools' },
...
function save() {
  ...
  ensurePermission(settings.quill).then(function (ok) {
    ...
    // Re-read the latest stored settings and merge ONLY the keys the popup owns
    // (skin / theme / hidden / quill). The popup snapshots settings at open time
    // and never refreshes, so writing the whole blob would clobber fields edited
    // on-page since then (thinkingOverride, panelPos, cardCollapsed, ...).
    chrome.storage.local.get([SETTINGS_KEY], function (data) {
      var base = (data && data[SETTINGS_KEY]) || {};
      base.skin = settings.skin;
      base.theme = settings.theme;
      base.hidden = settings.hidden;
      base.quill = settings.quill;
      var out = {}; out[SETTINGS_KEY] = base;
